...
class Evaluator:
    def __init__(self, name, eval_cfg, **kwargs):
        ...
        logger.info(
            "Evaluations stored in the experiment directory: %s",
            self.eval_cfg.output_dir,
        )
    ...
    def load_logs_from_file(self, file):
        ...
        if os.path.exists(file):
            logger.info("Loading existing evaluations from %s", file)
        ...
        for metric_name, metric_fn in self.metrics.items():
            if not overwrite and metric_name in logs and logs[metric_name]:
                ...
                if "agg_value" in logs[metric_name]:
                    logger.info(
                        "Result for metric %s:\t%f",
                        metric_name,
                        logs[metric_name]["agg_value"],
                    )
